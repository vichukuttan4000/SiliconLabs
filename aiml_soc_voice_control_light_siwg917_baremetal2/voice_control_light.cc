...
void handle_results(int32_t current_time, int result, uint8_t score, bool is_new_command) {
    ...
    if (is_new_command) {
        ...
        if (result == 0)
        {
            ...
            {
                case 0:
                    sl_si91x_simple_rgb_led_set_colour(DETECTION_ON, 0xFF0000); // RED
                    break;

                case 1:
                    sl_si91x_simple_rgb_led_set_colour(DETECTION_ON, 0x00FF00); // GREEN
                    break;

                case 2:
                    sl_si91x_simple_rgb_led_set_colour(DETECTION_ON, 0x0000FF); // BLUE
                    break;

                case 3:
                    sl_si91x_simple_rgb_led_set_colour(DETECTION_ON, 0xFFFFFF); // WHITE
                    break;
            }
